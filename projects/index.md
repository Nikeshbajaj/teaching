---
title: Projects | Nikesh Bajaj
layout: base
---
<br>
<br>
<br>

# Collaborative Projects with Students (UG/PG)

All the projects will be **Open Sourced** for reproducibility and wider impact.

<br>
<br>

<div class="wide-show">

<div style="width:90%; border:1px solid black;border-radius:20px 0 0 20px;margin:20px 0 20px 0;">
  <table>
  <tr>
  <td>
  <div class="column-box" style="width: 300px; margin:10px;">
    <div class="card" style="height: 280px;padding-top:5px;">
    <center><b>Project 1</b></center>
    <hr style="margin-top:5px;margin-bottom:0px;">
        <div class="card-body">
          <p><strong></strong> <a class="reference external" href="" target="_blank"></a></p>
          <a class="reference external image-reference" href="" target="_blank">
          <img alt="" src="" target="_blank" style="width: 300px;"/></a>
      </div>
    </div>
  </div>
  </td>
  <td style="vertical-align:top;">
  <!-- <hr> -->
  <br>
  <!-- <br> -->
  This project
  </td>
  </tr>
  </table>
</div>

</div>



<div class="narrow-show">

</div>

<br>
<br>
<br>
