...
function setup() {
  ...
  ThKnob = new MakeKnobC("green", 145, width/2+100, height/2+25, 0, 1, 0, 2,"Threshold", [0,0,0], 18);
  ThKnob.inactive();
  ...
  buttonProbRatio = createButton('Show/Hide P(c1)/P(c2)');
  ...
  // Resetbutton.position(cx+width-150, height/2+300);
  ...
  Th.position(cx,height/2-30);
  //Th.style('width', '450px');
  //Th.style('transform: rotate(270deg);');
  Th.input(ThresholdUpdate);
...
function ThresholdUpdate(){
  ThKnob.active();
  // ThKnob.knobValue = Th.value();
  ThKnob.updateValue(Th.value());
  //ThKnob.update();
  RedrawSamplr();
  ThKnob.inactive();
}
...
strokeWeight(4);
  ...
  rect(0, 0, 300, 200);
  strokeWeight(1);
  // fill(255-int(float(255)*A1),255,255,100)
  // rect(0, 0, 150, 100);
  // fill(255-int(float(255)*C1),255,255,100)
  // rect(150, 0, 150, 100);
  // fill(255-int(float(255)*C2),255,255,100)
  // rect(0, 100, 150, 100);
  // fill(255-int(float(255)*A2),255,255,100)
  ...
  // rect(150, 0, 150, 100);
  // fill(rr, 255-int(float(bb)*C2),255-int(float(gg)*C2),250)
  // rect(0, 100, 150, 100);
  // fill(rr, 255-int(float(bb)*A2),255-int(float(gg)*A2),250)
  // rect(150, 100, 150, 100);
  ...
  fill(0);
  textSize(16);
  text(THRESHOLD,width-50,  height/2-80);
}
...
class NormSampler{
  ...
  plotCurve(Ylevel){
    ...
    strokeWeight(2);
    //setLineDash([5, 5]);
    line(this.mapXtoI(this.mu), 0,this.mapXtoI(this.mu), height/2-100);
    //setLineDash([1, 1]);
